...
export default function App() {
    ...
    const [sortingDirections, setSortingDirections] = useState<{ [key: string]: SortingDirection }>({}); //
    ...
    useEffect(() => {
        fetchData().then((apiPeople) => {
            ...
            const ourFlattenedLocations = flattenLocations(
                apiPeople.map(({ location }: any) => location)
            );
            setFlattenedLocations(ourFlattenedLocations);
            const { headers } = ourFlattenedLocations;
            const ourSortingDirections: { [key: string]: SortingDirection } = {};
